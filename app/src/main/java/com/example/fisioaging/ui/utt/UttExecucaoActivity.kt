...
class UttExecucaoActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val tempoTotalEmMillis: Long = 30_000
    ...
    private fun salvarJSON() {
        ...
        val nomePac =
            paciente?.name?.replace(" ", "")
                ?: "Desconhecido"

        val emailPac =
            paciente?.email?.replace(" ", "")
                ?: "Desconhecido"

        val emailCodificado =
            URLEncoder.encode(emailPac, "UTF-8")

        val generoPac =
            paciente?.genre ?: "Não informado"
        ...
        val emailProfissional =
            sessionManager.fetchProfessionalEmail()

        val cnpjUnidade =
            sessionManager.fetchHealthUnitCnpj()
        ...
        val nomeArquivo =
            "UTT_${dataStr}_${horaStr}_${idPac}_${nomePac}_${emailCodificado}.json"
